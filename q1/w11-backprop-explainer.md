...
This is analogous to the path a heavy ball will take vs a lighter one. With a heavier ball there is more momentum that will carry the velocity forward along the axis of the valley. Mathematically the heavy-ball recurrence is defined as:
...
Since $0<\beta<1$ we weigh the more recent values more heavily than the older ones and the gradient components along the high-curvature eigendirection. The EMA will tend to cancel these out because they flip sign while the consistent component accumulates. Gradient components projected onto high-curvature eigendirections tend to flip signs each step because the curvature is large enough to make the gradient overshoot.
...
# Week 7 (v0.6): Momentum part 2

Momentum is a change of update rule, and does not change the metric or landscape. By including a 'memory' in the calculation, we can improve the convergence time over standard [[SGD]] as seen in the plot:
[w7s2-loss-plot.png]

## Momentum as EMA of gradients

As stated in the previous section, momentum can be expressed as an EMA of gradients, which has the effect of cancelling out the sign flipping of a steep ravine, while accumulating along the more consistent valley. This can be seen with [[eigendecomposition]] breaking the components of the steps to identify how the updates change with momentum.
...
With momentum, we see an improvement in convergence from $O(\kappa) \to O(\sqrt{\kappa})$. This is due to the fact that we shrink the error across all eignevalues at once. For a quadratic with exact gradients, this is the optimal method for shrinking error as shown by the Chebyshev recurrence.
...
# Week 11 (v0.8): Implicit Bias

Week 5 showed that the metric choice of gradient descent determines the direction of each step. When the loss is underdetermined ( $X \in \mathbb{R}^{n \times p}$ and $ p>n $) the metric will also determine where we end up. The zero-loss set is an affine subspace of dimension $p-n$, with every point minimizing the training loss. GD picks a specific point on this set, as determined by the metric.

Starting from $w_0 = 0$, GD converges to the minimum-$\ell_2$-norm solution $w^* = X^T(XX^T)^{-1}y$. The proof is the rowspace lemma:

1. The gradient $\nabla L = X^T(Xw-y)$ is a linear combination of rows of $X$, so it stays in $\text{rowspace}(X)$
2. $w_0 = 0$ is within rowspace trivially.
3. By induction, $w_k \in \text{rowspace}(X)$ for all $k$. The rowspace is orthogonal to the nullspace, so the rowspace point on the zero-loss subspace is $w^*$.

By corollary, if we start from a random initialization $w_0$, GD will converge to $w^* + w_{0,\perp}$, where $w_{0,\perp}$ is the projection of $w_0$ onto $\text{null}(X)$. GD never touches the nullspace component of the initialization.

However, an algorithm such as Adam will leave rowspace. The updates of Adam are equivalent to a non-trivial diagonal preconditioner being applied. This results in a different metric and leaves the rowspace, leading to a different solution point. We saw this with experimental data that identified a rowspace leak of 1.87 vs gradient descent at ~10^-15 when compared to the closed form solution, and the norms differed: $\|w_{Adam}\|_2 = 3.24$ vs $\|w^*\|_2 = 2.65$.

Ultimately this means that the choice of optimizer is a modeling decision, not just a mechanism for finding the solution. A different optimizer leads to a different solution shape. From this perspective, AdamW's decoupled weight decay can be seen as an explicit correction to Adam's implicit bias when the bias is pointing the wrong way for generalization. As we've tried to explain generalization behavior, we've ruled out static (loss surface) as the underlying reason. This leaves the dynamic explanation the trajectory, not the geometry of where we land. Implicit bias is one instance of this view. The algorithm's metric determines not just the path, but the destination, and the destination is what generalizes. Which specific norm is minimized with GD on neural networks is an open question. From the linear case we know some norm is being minimized; which norm GD picks on deep networks is the open question.
