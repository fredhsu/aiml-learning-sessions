...
def _(mo):
    mo.md(r"""
    # Week 8 Session 2 - Regularization
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Observations
    Weight decay helped with momentum SGD to tame the overshooting effect of high momentum by pulling the weights towards zero. It did not have a strong effect on vanilla SGD, since the ravine jumping is not something that pulling towards zero affects, it doesn't affect the path as much as the destination. However, this dampening effect is an indirect result of shirinking the parameters which shrink the gradients since gradients scale weight weights in this quadratic. It is not intended as a fix for velocity accumulation. In both cases I saw the decay roughly halved the norm by the end of the run.
    """)
    return
